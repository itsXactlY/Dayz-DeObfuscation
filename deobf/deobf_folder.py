...
"""
    ...
    def is_valuable_code(self, content: str) -> bool:
        ...
        valuable_indicators = {
            ...
            # DayZ/Enfusion specific indicators
            'has_dayz_patterns': bool(re.search(r'(GetGame|GetMission|IsMultiplayer|IsServer|IsClient|Cast)\s*\(', content)),
            'has_ref_patterns': bool(re.search(r'ref\s+\w+', content)),
            
            # Obfuscation patterns
            'has_complex_identifiers': len([id for id in self.extract_and_filter_identifiers(content) if len(id) > 10]) > 2,
            'has_mixed_case_pattern': bool(re.search(r'\b[A-Z][a-z]+[A-Z][A-Za-z]+\b', content)),
        }

        # Content quality checks
        code_lines = [line.strip() for line in content.split('\n') if line.strip()]
        if len(code_lines) < 3:  # Skip very short files
            return False

        # Calculate meaningful code ratio
        ...
        code_density = meaningful_chars / total_chars if total_chars > 0 else 0

        # Check for junk patterns
        junk_indicators = {
            'repeated_patterns': len(set(code_lines)) / len(code_lines) < 0.3 if code_lines else False,
            'excessive_numbers': len(re.findall(r'\d+', content)) / len(code_lines) > 5 if code_lines else False,
            'random_looking_strings': len(re.findall(r'"[A-Za-z0-9]{20,}"', content)) > 0,
            'single_char_vars': len(re.findall(r'\b[a-z]\b', content)) > len(code_lines) * 0.5,
        }

        # Scoring system
        valuable_score = sum(valuable_indicators.values())
        junk_score = sum(junk_indicators.values())

        # Debug information
        print("\nCode Analysis:")
        print("Valuable indicators:", {k: v for k, v in valuable_indicators.items() if v})
        print("Junk indicators:", {k: v for k, v in junk_indicators.items() if v})
        print(f"Code density: {code_density:.2f}")
        print(f"Scores - Valuable: {valuable_score}, Junk: {junk_score}")

        # Decision making
        is_valuable = (
            valuable_score >= 3 and  # At least 3 valuable indicators
            junk_score <= 1 and     # Maximum 1 junk indicator
            code_density > 0.4      # At least 40% meaningful code
        )
    ...
    async def process_folder(self, input_path: str, output_path: str):
        ...
        print("Starting folder analysis...")
        
        # First pass: Count files
        for root, _, files in os.walk(input_path):
            for file in files:
                if file.endswith(('.c', '.cpp', '.h', '.hpp', '.cs', '.sqf')):
                    total_files += 1

        print(f"\nFound {total_files} total code files")
            ...
            for file in files:
                if file.endswith(('.c', '.cpp', '.h', '.hpp', '.cs', '.sqf')):
                    ...
                    try:
                        ...
                        # Try multiple encodings aggressively
                        content = None
                        for encoding in ['utf-8', 'latin1', 'cp1252', 'ascii', 'iso-8859-1', 'utf-16', 'utf-32']:
                            try:
                        ...
                        # If no encoding worked, use latin1 as fallback (it always works)
                        if content is None:
                            content = content_bytes.decode('latin1')

                        # Process the file regardless of content type
                        processed_files += 1
                        print(f"\nProcessing file ({processed_files}/{total_files}): {input_file}")
                        ...
                        # Write output with original encoding if possible, fallback to utf-8
                        try:
                            with open(output_file, 'wb') as f:
                                f.write(deobfuscated.encode('utf-8', errors='replace'))
                            print(f"Saved processed file: {output_file}")
                        except Exception as e:
                            print(f"Using fallback save method for {output_file}")
                            with open(output_file, 'wb') as f:
                                f.write(deobfuscated.encode('latin1', errors='replace'))
                        
                    except Exception as e:
                        print(f"Error processing {input_file}: {str(e)}")
                        # Try to copy the file as-is if processing fails
                        try:
                            rel_path = os.path.relpath(root, input_path)
                            output_dir = os.path.join(output_path, rel_path)
                            os.makedirs(output_dir, exist_ok=True)
                            ...
                            with open(input_file, 'rb') as src, open(output_file, 'wb') as dst:
                                dst.write(src.read())
                            print(f"Copied original file: {output_file}")
                        except Exception as copy_error:
                            print(f"Failed to copy {input_file}: {str(copy_error)}")
...
async def main():
    input_folder = "/home/alca/Schreibtisch/test/AdvancedGroups_Server_latest/"  # Replace with your input folder
    output_folder = "/home/alca/Schreibtisch/test/algo_cleaned/"  # Replace with your output folder
